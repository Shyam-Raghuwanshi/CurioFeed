 ...
 * 
 * This module handles content crawling using the Firecrawl API to fetch real-time web content
 * based on user interests. It searches across the web for relevant articles and formats them
 * into a consistent structure for the feed.
 * 
 * Features:
 * - Real-time web content crawling via Firecrawl API
 * - Interest-based search queries (Tech, Design, Business, Health, Finance, Other)
 * - Automatic content formatting and deduplication
 * - Retry logic for robust API interactions
 * - Error handling with meaningful error messages
 * 
 * Requirements:
 * - VITE_FIRECRAWL_API_KEY environment variable must be set
 * - Active Firecrawl API subscription
 */
...
export const testCrawling = async (): Promise<void> => {
  ...
  for (const interest of ['Tech', 'Design'] as Interest[]) {
    ...
    try {
      ...
      console.error(`Failed to crawl ${interest}:`, error);
    }
  }
};